...
- `<work_branch>` = problem.md の `**Issue ID:**` と config.json の `branch_name_template` から解決。どちらかがなければ `tdd/<project>`
- `<plans_dir>` = `<meta>` != `<work_repo_abs>` なら `<meta>/plans/<project>`、同じなら `<wt>/plans/<project>`

worktree がなければ再追加する:

```bash
if [ ! -d "<wt>" ]; then
  git -C <work_repo_abs> worktree add <wt> <work_branch>
fi
```
...
worktree（`<wt>`）は「読み込み」セクションで確定済み。`<meta>` と `<work_repo_abs>` が同じか異なるかで処理を分岐する。

**単一レポ構成（`<meta>` == `<work_repo_abs>`）:**

最新ファイルを worktree に同期してからアーカイブする:
...
**サブレポ構成（`<meta>` != `<work_repo_abs>`）:**

plans は meta_repo 内に留まっているため、meta_repo 側でアーカイブする:

```bash
mkdir -p <meta>/plans/archives
mv <meta>/plans/<project>/ <meta>/plans/archives/<project>/
```

`commit_plans` が `true` であれば meta_repo にコミットする:
...
`commit_plans` が `false` の場合はコミットしない（いずれの構成でも）。

worktree を削除する（ブランチは残す）:
...
```
アーカイブは `<work_branch>` ブランチにコミットしました。
```

ユーザーへ報告する（サブレポの場合）:

```
アーカイブは meta リポジトリにコミットしました。
```
