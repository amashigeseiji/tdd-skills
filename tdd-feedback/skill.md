...
`problem.md` の `**作業レポジトリ:**` フィールドを確認する:

```bash
grep "^\*\*作業レポジトリ:" plans/<project>/problem.md
```

- **フィールドがある場合**: 以降の worktree 操作はすべて `./<value>/` を起点に行う（`./tdd/<project>` は `./<value>/tdd/<project>` と読み替える）
- **フィールドがない場合**: 現在のディレクトリを起点にする（従来どおり）
...
worktree を追加し、最新ファイルを同期してからアーカイブする。
`<worktree>` は `読み込み` セクションで確定した起点（`./tdd/<project>` または `./<repo>/tdd/<project>`）:

```bash
# worktree がなければ追加（作業レポジトリがサブディレクトリの場合は cd してから実行）
if [ ! -d "<worktree>" ]; then
  # フィールドなし: git worktree add ./tdd/<project> tdd/<project>
  # フィールドあり: cd ./<repo> && git worktree add ./tdd/<project> tdd/<project> && cd ..
fi

# 最新の plans/<project>/ と docs/dictionary.md を worktree に同期
mkdir -p <worktree>/plans/archives
cp -r plans/<project>/. <worktree>/plans/<project>/
cp docs/dictionary.md <worktree>/docs/dictionary.md 2>/dev/null || true

# worktree 内でアーカイブ先に移動
mv <worktree>/plans/<project>/ <worktree>/plans/archives/<project>/
```

`.claude/tdd/config.json` の `commit_plans` が `true` であれば、worktree 内でコミットする:

```bash
git -C <worktree> add plans/archives/<project>/
git -C <worktree> add -u
git -C <worktree> commit -m "archive: <project>"
```
...
```bash
# フィールドなし: git worktree remove ./tdd/<project>
# フィールドあり: cd ./<repo> && git worktree remove ./tdd/<project> && cd ..
```
