...
mkdir -p ./tdd/<project>/plans/archives
cp -r plans/<project>/. ./tdd/<project>/plans/<project>/
cp docs/dictionary.md ./tdd/<project>/docs/dictionary.md 2>/dev/null || true

# worktree 内でアーカイブ先に移動
mv ./tdd/<project>/plans/<project>/ ./tdd/<project>/plans/archives/<project>/
```

`.claude/tdd/config.json` の `commit_plans` が `true` であれば、worktree 内でコミットする:

```bash
git -C ./tdd/<project> add plans/archives/<project>/
git -C ./tdd/<project> add -u
git -C ./tdd/<project> commit -m "archive: <project>"
```
