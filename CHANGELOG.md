...
- **`tdd-run` を単一文書（711行）から、ルーター＋2サブコマンド＋3規範文書に分割** —
  現行文書の軸が「ステップの時間順」一本しかなく、新規要件の多くがステップ横断的な規範
  （命名・確認の取り方・評価基準）であるため、規範が特定ステップの中に埋め込まれ、同種の規範が
  別ステップに別の言い回しで埋まり、全体の整合性が見えなくなっていた。
  構成は `tdd-run/skill.md`（ルーター）、`subcmds/decompose.md`（分解）、
  `subcmds/compose.md`（合成）、`norms/{vocabulary,dialogue,pattern-matching}.md`。
  分割の基準は実行順ではなく**変更の種類** — 新規要件が来たときに置き場所が一意に決まること。
  旧文書は `docs/archive/tdd-run-skill-2026-07-29.md` に保存。
- **ステップ番号による進行を、`test-tree.md` の有無によるルーティングに置き換え** —
  `plans/<project>/test-tree.md` がなければ分解、あれば合成へ入る。feedback からの構造的再入は
  分解の「部分木再入口」へ入り、ツリー編集後に合成へ合流する。
  **`test-tree.md` の存在は常に「確認済み」を意味する**（draft 段階のツリーはファイルにしない）。
  合成の現在位置はテストスイートの通過状態から導出し、進捗を別途記録しない。
  セッションを分解と合成で分けて実行する運用が、追加の取り決めなしに成立する。
- **インターフェース確定を合成（統合ステップ）から分解へ移した** — 開き戸か引き戸かの決定は
  必要な部品の集合＝ツリーの形を変える設計判断であり、できあがった装置の組み込み時に発生する
  問題ではない。分解の候補が同じサブ概念を異なるアルゴリズムで実現しているだけか、異なる
  ドメインレベルのサブ概念そのものを要求しているかを判定し（両候補を一段だけ描いて子ノードの
  集合が変わるかを見る）、後者なら決定しないまま分解を進めない。決定は**構造的仮登録**
  （`src: null`・wip、インターフェースの形と関係のみ）として辞書に書き、合成の組み込みで
  `src` を充填して完成させる。`ui` エントリの `relations` 意味論・登録の基準・`states`・
  `affordances` の規約も分解側へ移動した。意匠（見た目・design-token・`src` の確定）は
  従来どおり合成の組み込みに残る。
- **合成の不通過時の対処を、回数カウンタから徴候ベースの診断規則に変更** — 「5回連続失敗」の
  ループ防止規則を削除し、徴候で判定する: テストを満たすのにインターフェースが提供していない
  情報や能力が必要なら**インターフェースの反証**、実装がどこにもテストのない設計判断を含んで
  いるなら**隠れた子ノード**。どちらも合成の流れの一拍としての局所修繕であり、再入ではない。
  修繕が確認済みツリーの形に触れそうになったら、自律的に再構築せず徴候を提示して停止する。

### Added

- **不可逆性チェックを語彙規範に追加** — 概念の述語が行為であるとき、「この行為を取り消したとき
  系全体は取り消す前と区別不能な状態に戻るか」（実用的な言い換え: イベントソーシングで実装したら
  意味があるか）を登録前に問う。不可逆なら解釈を確定してから登録する（先例 → problem.md /
  user-story.md → 人間）。併せて、**「述語は挙動であって手段ではない」が成り立つのは行為が
  可逆だと確認できている場合に限る**という前提を明示した。
- **技術語スキャンに「要件レベルの事実まで一緒に消さない」注記を追加** — 述語から技術語を
  除去するとき、その技術語に乗っていた要件レベルの事実（例: リロードをまたいで状態が生き残る）
  まで道連れに消してはならない。その事実は当該ノードの述語に残すか、別のノードに属させる。
- **分解の調査ステップを「最小限の探索」から「分解仮説の材料集め」に格上げ** — 辞書と実装の
  両方を並行して調べる（辞書は改訂されながら運用されるため、実装にのみ先例が残ることがある）。
  確認事項に「この問題は既存の配線済みモジュールの修正で解決できないか」を追加し、拡張可能な
  既存モジュールが見つかった場合は root X をその既存概念として新しい名前を作らない。
- **分解に単一エントリーポイントのスコープを明記** — 一回の実行は単一のエントリーポイントを
  扱う。problem.md が複数を要求する場合は実行を分ける。

### Removed

- **突合ウォークスルー（旧ステップ9）を対応表の目視確認まで縮約** — `@vocab` / `describe()` /
  実装名 / `src` の対応検査は lint として機械化する予定で、材料化と組み込みの時点で規約を
  守っていれば人手の突合は不要になる。**lint 整備までの暫定手順**であることを本文に明記した。
  依存グラフ検査（孤立ノード検査・配線検査）はツーリングの注記に圧縮した。
- **命名機会の三箇条**（"a flow of ~" / 共有ノード / "one phrase"）を削除。
  型と関数シグネチャの節は残した。
- **problem.md の旧フィールド `**作業ディレクトリ:**` の移行注記を削除**
  （`find-config.sh` 側の解決パスは存置）。

### Migration

ユーザー側の対応は不要。スキルはディレクトリごと symlink されるため、`git pull`（または
`/tdd-update`）で新しい `subcmds/` と `norms/` が配置される。既存の `plans/<project>/` の
成果物形式に変更はない。
