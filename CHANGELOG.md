...
### Added
- **辞書に `ui`/`ui-pattern`/`design-token` の3ドメインと `states`/`affordances` フィールドを追加** —
  ...
  安定させられるようにした。`domain: "ui"` エントリーは常に操作ハンドルであり、複数のハンドルが
  合成されてより大きなハンドルになる（`relations: contains`。器が中身を保持する関係ではない）。
  概念エントリー側の `affordances` は概念が許す操作をドメイン言語として列挙し、`references` だけ
  では区別できない「どの操作を起動するUIか」を明示できるようにする。
- **`tdd-run` ステップ8にUI要素の命名・再利用の手続きを追加** — UIは機械（できるツリー）の分解
  には属さないシグニファイアであり、実際にエントリーポイントへ組み込む瞬間に初めて実体を持つ。
  エントリーポイントに部品を組み込むたびに、①既存の `domain: "ui"` エントリを検索して
  再利用できないか確認 → ②該当する `design-token` があれば適用（リテラルな色・px値を避ける）
  → ③再利用できない新規部品のみ命名・登録 → ④配線を `depgraph-search.js` で機械チェック
  → ⑤複数エントリが揃ったら `ui-pattern` 照合、という手順にした。エントリーポイントそのもの
  （画面・ページ等）はできるツリーのroot Xと同じ理由（分解・合成の起点）で常に登録し、それ以外の
  複合ハンドルは他の文脈でも再利用できるときにのみ名付ける。
...
- **`/tdd-vocab plan` が `affordances`/`states` の候補も拾うように** — 既存の「何が」「何を」
  「どうする」抽出はproblem.mdに既にある情報を素材にしており、対象概念への操作や状態を表す語も
  同じ文面に含まれていることが多い。これを拾わずtdd-runのステップ8まで持ち越すのは、既にある
  情報を一度捨てて後で拾い直しているだけだった。手順3の抽出に、概念の目的語になっている操作は
  `affordances` 候補、概念の記述に現れる状態を表す語は `states` 候補として拾う一文を追加した。
