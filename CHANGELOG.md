...
- **`tdd-run` に `ui` エントリーの配線を機械チェックする手順を追加**（ステップ7.5） —
  登録した `ui` エントリーが実際に対象概念を操作しているかは目視確認に頼るしかなく、
  見落としのリスクがあった。application/solutionドメイン向けに既にある `src`・`@vocab`・
  依存グラフの孤立ノード検出の仕組みを `ui` にも転用し、`relations` で指した概念への依存が
  コード上にあるかを機械的に確認できるようにした。UI部品を独立モジュールに切り出せるかは
  実装機構に依存するため、複数の `ui` エントリーが同じ `src` を共有する場合はこのチェックの
  対象外とする。
- **`tdd-run` ステップ8にUI意図との整合確認を追加** — 配線の有無はステップ7.5の機械チェックに
  委ね、ステップ8では実際にアプリを動かした上でのみ確認できる見た目・states依存の表示切り替え
  だけを確認する。
