...
セッション2: 実装（対話モード）
  ...
  → テスト（「できる」のツリー）+ 実装（利用可能な状態）+ 受け入れテスト骨格
...
セッション3: リファクタリング（任意）
  /tdd-refactor
  → 辞書・テスト・実装を横断して観察し、変更を実施
  → 複数 run の後はコードベース全体を横断して B の同一性を探す
...
tests/
  acceptance/
    <project>.spec.ts        # 受け入れテスト骨格（/tdd-userstory run）
```
...
| /tdd-userstory | BDD仕様の生成・受け入れテスト骨格の実行 | problem.md の変更・アサーションの自動生成・テストの green 化 |
