...
**目的:** 新しいリポジトリにTDDスキルを初めて導入するときに一度だけ実行する

- plans コミット設定を確定し `.claude/tdd/config.json` に保存する
- No の場合は `plans/` を `.gitignore` に追加する
- `/tdd-scaffold` を呼び、`.claude/tdd/scaffold.sh` を生成する
- `/tdd-vocab init` を呼び、`docs/dictionary.md` を初期化する
- **成果物:** `.claude/tdd/config.json`、`.claude/tdd/scaffold.sh`、`docs/dictionary.md`
...
| 呼び出し | タイミング | 目的 |
|---------|-----------|------|
| `/tdd-vocab init` | 既存プロジェクト導入時（`/tdd-init` 経由）・BC が増えたとき | コードベースを俯瞰してコンテキストを発見し、語彙を抽出して `docs/dictionary.md` を作る |
...
初回（既存プロジェクト導入時）:
  ...
  → .claude/tdd/scaffold.sh（スタブ生成スクリプト）
...
.claude/
  tdd/
    config.json              # plans コミット設定（/tdd-init が生成）
...
| /tdd-init | plans コミット設定・scaffold・vocab init の初期化（初回のみ） | 2回目以降の実行（vocab init は直接呼ぶ） |
