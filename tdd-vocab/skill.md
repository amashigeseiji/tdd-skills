...
**語彙はアプリケーションドメイン・ソリューションドメイン・役割・パターン・UI・UIパターン・デザイントークンの7層に分かれます。**
...
- **役割** (`domain: "actor"`): 「できる」のツリーで**主語に使う行為の担い手**（「作成者」「閲覧者」等）。ノードは `<主語> は <可能形で終わる述語>` の形で書かれ、その主語がこれにあたる。他の層のエントリが「何であるか・何ができるか」を持つのに対し、役割は**誰の行為可能性としてツリーを立てるか**を定める。薄いエントリでよい——定義だけを持ち、`src` は `null`、**操作の一覧は持たせない**（操作は行為の対象であるオブジェクト側の `affordances` に属する）。tdd-run の分解で役割主語の root を書くときに、未登録なら通常の語彙登録ルール（表＋承認）で登録する。
...
- **UI** (`domain: "ui"`): 操作ハンドル。概念を`references`する（例: 削除ボタンが記事を参照する）、またはハンドル同士が合成されてより大きなハンドルになる（`contains`、例: 入力ハンドルと送信ハンドルが合成されて投稿フォームになる。器が中身を保持する関係ではない）。`context` を持ち、属するBCに紐づく。空間的な配置は書かない。tdd-run の分解（インターフェース確定）で構造的仮登録（`src: null`、インターフェースの形と関係のみ）として作られ、合成の組み込みで `src` が充填され design-token が適用されて完成する。
- **UIパターン** (`domain: "ui-pattern"`): master-detail のような、一連の操作を支える構造パターン。`pattern` と同じ形（`heuristic` 必須 + `components` で役割名）だが、照合対象が違う——`pattern` はテストツリーの形、`ui-pattern` は `ui` エントリ同士の関係の形。`context: null`。tdd-run の合成の組み込みでUIエントリが複数蓄積した時点で照合する。
...
**`src` フィールドの所有権:** `src` は `tdd-run` が書き込む代表1ファイルの簡易キャッシュ。`tdd-vocab` の各サブコマンドはこれを読むだけで書き込まない。概念の実装箇所を1件だけ確実に知りたい場合は `grep -rn "@vocab: <概念名>"` する（多対多で正式、`check` が整合性を保証する）。
