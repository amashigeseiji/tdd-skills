...
**語彙はアプリケーションドメイン・ソリューションドメイン・パターンの3層に分かれます。**
...
`domain: "pattern"` のエントリー。`context: null` で登録する。
...
```json
{
  ...
  "context": null,
  "domain": "pattern",
  "definition": "有限個の状態とイベント駆動のトランジションで振る舞いを管理する構造パターン。",
  "heuristic": "複数の状態ノードがあり条件によって遷移している",
  "components": [
    { "name": "状態", "role": "振る舞いが切り替わる単位" },
    { "name": "イベント", "role": "状態遷移を引き起こすトリガー" },
    { "name": "トランジション", "role": "イベントによる状態の移行" }
  ]
}
```

- `heuristic`: tdd-run がテストツリーと照合する手がかり。登録時に必ず書く。
- `components`: 構成要素と役割。独自パターンは必須、既知パターンは省略可。WIP 登録時は省略し、promote 時に補完してよい。
